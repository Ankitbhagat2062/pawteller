...
export async function POST(request: Request) {
  try {
    ...
    if (existing) {
      return NextResponse.json({ ok: false, message: "Admin email is already registered." }, { status: 400 });
    }

    const passwordHash = await bcrypt.hash(password, 10);

    const admin = new AdminModel({ adminEmail, passwordHash, resendApiKey, mongodbUri });
    await admin.save();

    const token = createAdminToken({ adminEmail });
    return NextResponse.json({ ok: true, message: "Registration successful. You may now log in.", token });
  } catch (e: unknown) {
    return NextResponse.json({ ok: false, message: e instanceof Error ? e.message : "Registration failed." }, { status: 500 });
  }
}
