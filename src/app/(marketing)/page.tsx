      ...
      <div className="w-full font-sans text-slate-900 dark:bg-zinc-950 dark:text-slate-50">
        ...
        >
          ...
          {heroSection?.map((hero: HomepageHeroCms) => (
            <section
              key={hero.id}
              className="relative rounded-2xl left-1/2 mt-6 w-full -translate-x-1/2 overflow-hidden bg-[#f3ded3] px-6 py-16 sm:px-8 md:px-12 lg:px-16 lg:py-20 dark:rounded-3xl dark:bg-emerald-500/10"
            >
              <div className="relative mx-auto grid w-full max-w-7xl gap-12 lg:grid-cols-12 lg:items-center">
                ...
                uppercase tracking-[0.28em] text-[#3f342f] dark:bg-emerald-500/10 dark:text-emerald-300"
                  ...
                  {(() => {
                    const title = hero.h1
                      ? hero.h1
                      : "Smart Calculators for your best friend";
                    const ArrayOfParts: string[] = title.trim().split(/\s+/); // Removes the full match element

                    if (title) {
                      return (
                        <h1
                          className="mt-8 max-w-2xl font-[Georgia,serif] text-[3rem] font-black leading-[0.9] tracking-normal 
                text-[#2a1b15] dark:text-[#ad7e6b] sm:text-[5.25rem] lg:text-[5.6rem]"
                        >
                          ...
                          <span className="block">
                            {ArrayOfParts[2] ?? ""}{" "}
                            <span
                              className={`font-[Georgia,serif] font-black italic text-[#315846] dark:text-[#a6d4bd]`}
                            >
                              {ArrayOfParts[3] ?? ""}
                            </span>
                          </span>
                          <span
                            className={`block font-[Georgia,serif] text-[0.9em] font-black italic text-[#315846] dark:text-[#a6d4bd]`}
                          >
          ...
          {featureSection && (
            ...
            >
              <h2 id="featured-calculators-title" className="sr-only">
                Featured calculators
              </h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:auto-rows-[285px] lg:gap-5">
                {featureSection?.calculators?.map((calc: CalculatorProps) => {
            ...
            >
              {breedQuiz.map(
                (quiz: {
                  ...
                  <div
                    key={quiz.eyebrow}
                    className="overflow-hidden rounded-[28px] bg-[#e4c99a] px-6 py-10 shadow-none ring-1 ring-transparent sm:rounded-[34px]
