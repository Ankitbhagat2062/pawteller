...
import { ArrowRight, ArrowLeft, Dog, RotateCcw } from "lucide-react";
...
export function QuizComponent() {
  ...
  const handleOptionSelect = (option: string) => {
    ...
    setTimeout(() => {
      if (currentStep < quizData.totalQuestions - 1) {
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 sm:p-6 md:p-8">
        <div className="w-full max-w-2xl text-center">
          ...
          <p className="text-muted-foreground mb-8">
            Based on your answers, we&apos;d recommend breeds that match your lifestyle.
          </p>

          <div className="bg-card rounded-2xl p-6 sm:p-8 shadow-sm border border-border mb-8">
            <h2 className="text-xl font-semibold mb-4 text-card-foreground">Your Answers:</h2>
            <div className="space-y-3 text-left">
              {quizData.steps.map((step, index) => (
                <div key={index} className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-4 py-2 border-b border-border last:border-0">
                  <span className="text-muted-foreground text-sm">{step.question}</span>
                  <span className="font-medium text-card-foreground">{selectedAnswers[index]}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="px-4 sm:px-6 md:px-8 max-w-4xl mx-auto w-full">
        <div className="flex justify-between items-center mb-2 text-sm text-muted-foreground">
          <span>Step {currentStep + 1} of {quizData.totalQuestions}</span>
          <span>{progress}%</span>
        </div>
