...
const DogAgeCalculator = () => {
  ...
  const [ageDescription, setAgeDescription] = useState<string>('Your dog is entering their senior years. Regular vet checkups and a comfortable environment are important.');
// 
  // Dog aging formula based on veterinary research
  const calculateHumanAge = (age: number, size: string): { age: number; category: string; description: string } => {
    let humanYears = 0;
    let category = '';
    let description = '';

    if (age <= 0) {
      return { age: 0, category: 'Newborn', description: 'Your puppy is just born!' };
    }
    ...
    if (age === 1) {
      ...
      description = 'Your dog is playful and learning quickly. This is the perfect time for training!';
    } else if (age < 3) {
      humanYears = 15 + (age - 1) * 9;
      category = 'Puppy';
      description = 'Your dog is in their prime growing years. Continue training and socialization.';
    } else {
      ...
      if (humanYears < 20) {
        category = 'Young';
        description = 'Your dog is energetic and playful. Provide plenty of exercise and mental stimulation.';
      } else if (humanYears < 35) {
        category = 'Young Adult';
        description = 'Your dog is in their prime years. This is an ideal time for training and activities.';
      } else if (humanYears < 50) {
        category = 'Mature Adult';
        description = 'Your dog is mature and more stable. They may prefer moderate exercise and comfort.';
      } else if (humanYears < 65) {
        category = 'Senior';
        description = 'Your dog is entering their senior years. Regular vet checkups and a comfortable environment are important.';
      } else if (humanYears < 80) {
        category = 'Geriatric';
        description = 'Your dog is a senior. Provide soft bedding, easy access to food and water, and extra attention.';
      } else {
        category = 'Venerable';
        description = 'Your dog has reached an impressive age! Focus on comfort, gentle care, and quality time together.';
      }
    }
        ...
        <main className="max-w-6xl mx-auto px-4 py-8 md:py-12">
          ...
          <section className="mb-12">
            <div className="text-center mb-8">
              <h2 className="text-3xl md:text-4xl font-bold text-balance text-foreground mb-4">
                How old is your dog in <span className="italic text-primary">human years</span>?
              </h2>
              ...
              <div className="flex flex-wrap gap-2 justify-center text-sm text-muted-foreground">
                <span className="px-3 py-1 rounded-full bg-accent/10 text-accent">Science-backed</span>
                <span className="px-3 py-1 rounded-full bg-primary/10 text-primary">Veterinary approved</span>
                <span className="px-3 py-1 rounded-full bg-secondary/10 text-secondary">Size-accurate</span>
              </div>
            </div>
          </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card className="p-6 md:p-8 bg-card border-border/60 shadow-sm hover:shadow-md transition-shadow">
              <h3 className="text-xl font-semibold text-foreground mb-6">Enter Your Dog&apos;s Details</h3>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Select value={dogSize} onValueChange={handleSizeChange}>
                    <SelectTrigger id="size" className="w-full bg-input border-border text-foreground">
            ...
            <Card className="p-6 md:p-8 bg-gradient-to-br from-primary/5 to-secondary/5 border-primary/20 shadow-sm hover:shadow-md transition-shadow">
              <div className="text-center">
                ...
                <div className="mb-6">
                  <p className="text-6xl md:text-7xl font-bold text-primary mb-2">{humanAge}</p>
                  <p className="text-xl font-semibold text-foreground">{ageCategory}</p>
                </div>
                ...
                <div className="bg-primary/10 rounded-lg p-4 border border-primary/20">
                  <div className="flex gap-3 items-start">
                    ...
                    <p className="text-sm text-foreground leading-relaxed">{ageDescription}</p>
                  </div>
                </div>
              </div>
            </Card>
          ...
          <section className="mt-16 space-y-8">
            <div className="text-center mb-10">
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3">Why Size Matters</h2>
              ...
              {[
                {
                  ...
                  color: 'from-blue-500/20 to-blue-600/20 border-blue-200/50 dark:border-blue-800/50',
                },
                {
                  size: 'Medium',
                  example: 'Beagle, Cocker Spaniel',
                  advantage: 'Balanced aging pattern, standard lifespan',
                  color: 'from-green-500/20 to-green-600/20 border-green-200/50 dark:border-green-800/50',
                },
                {
                  size: 'Large',
                  example: 'Golden Retriever, German Shepherd',
                  advantage: 'Mature faster, shorter overall lifespan',
                  color: 'from-orange-500/20 to-orange-600/20 border-orange-200/50 dark:border-orange-800/50',
                },
                {
                  size: 'Giant',
                  example: 'Great Dane, Saint Bernard',
                  advantage: 'Age fastest, require extra care',
                  color: 'from-red-500/20 to-red-600/20 border-red-200/50 dark:border-red-800/50',
                },
              ].map((item, idx) => (
                <Card key={idx} className={`p-4 bg-gradient-to-br ${item.color} border`}>
                  <h4 className="font-semibold text-foreground mb-2">{item.size}</h4>
                  <p className="text-xs text-muted-foreground mb-3">{item.example}</p>
                  <p className="text-sm text-foreground leading-relaxed">{item.advantage}</p>
                </Card>
              ))}
            </div>
          </section>
          ...
          <section className="mt-16 max-w-3xl mx-auto">
            ...
            <div className="space-y-4">
              {[
                ...
                <Card key={idx} className="p-4 md:p-6 border-border/60 hover:border-border transition-colors">
          ...
          <section className="mt-16 text-center">
            <Card className="p-8 md:p-12 bg-gradient-to-r from-primary/10 to-secondary/10 border-primary/30">
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-4">Keep Your Dog Healthy</h2>
