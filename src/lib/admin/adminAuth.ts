...
export function createAdminToken(payload: { adminEmail: string; role?: string; exp?: number }) {
...
export function verifyAdminToken(token: string) {
  if (!token.startsWith(TOKEN_PREFIX)) return { ok: false as const, reason: "INVALID" };
  ...
  const ok = crypto.timingSafeEqual(Buffer.from(sigPart), Buffer.from(expectedSig));
  if (!ok) return { ok: false as const, reason: "INVALID_SIG" };

  let body: any;
  try {
    body = JSON.parse(Buffer.from(bodyPart.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString("utf8"));
  } catch {
    return { ok: false as const, reason: "INVALID_PAYLOAD" };
  }

  if (typeof body?.exp !== "number") return { ok: false as const, reason: "NO_EXP" };
  if (Math.floor(Date.now() / 1000) > body.exp) return { ok: false as const, reason: "EXPIRED" };

  return { ok: true as const, payload: { adminEmail: body.sub as string, role: body.role as string } };
}
